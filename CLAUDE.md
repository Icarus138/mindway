...
- Citations : 102 en stock, **21 signées « Mindway » exclues de tous les tirages** (`QA`). Ids = strings (`"q62"`). Contextes par catégorie (`Q_CTX`). Focus/bilan seedés par jour (`qPick`), validation aléatoire (`qValid`, ≤100 car., scène `#vwin`).

## Modèle de données — NE JAMAIS CASSER

localStorage `mindway_v2`, fallback mémoire si indisponible.
- `D.days[iso]` : `{mit, tasks[], quick[], mindset{clarte,energie,ctx[],note}, timer, plan, ritualsDone[], review, off, startedHour}`
...
- Insights verrouillés < 7 jours clôturés ; 4 règles actives. **Ce verrou J7 est le futur point de conversion Plus — ne pas le retirer.**
