...
- Citations : 130 en stock, **21 signées « Mindway » exclues de tous les tirages** (`QA`). Élargissement v3.11 (littérature, sciences humaines, incertitude) : **toute attribution est vérifiée contre une source primaire avant intégration** — une citation inventée ou mal attribuée est inacceptable. Ids = strings (`"q62"`). Contextes par catégorie (`Q_CTX`). Focus/bilan seedés par jour (`qPick`), validation aléatoire (`qValid`, ≤100 car., scène `#vwin`).
